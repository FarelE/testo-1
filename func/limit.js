...
exports.isLimit = function(sender, limitCount, _db){
    let found = false
    for (let i of _db) {
        if (i.id === sender) {
            let limits = i.limit
    ...
    if (found === false) {
        ...
        _db.push(obj)
        fs.writeFileSync('./home/database/limit.json', JSON.stringify(_db, null, 2))
        return false
    }
}
...
exports.kurangLimit = function(sender, limit, _db){
    let position = false
    Object.keys(_db).forEach((i) => {
        if (_db[i].id === sender) {
            position = i
...
exports.addBalance = function(sender, duit, _db){
    ...
    if (position !== false) {
        ...
        fs.writeFileSync('./home/database/balance.json', JSON.stringify(_db, null, 2))
    } else {
        const bulin = ({
            id: sender,
            balance: duit
...
exports.isGame = function(sender, isOwner, gcount, _db){
    ...
    if (found === false){
        ...
        _db.push(obj);
        fs.writeFileSync('./home/database/glimit.json',JSON.stringify(_db, null, 2));
        return false;
    }
}
...
exports.createHit = function(sender, _db){
    ...
    _db.push(anohoh);
    fs.writeFileSync('./home/database/userhit.json',JSON.stringify(_db, null, 2));''
