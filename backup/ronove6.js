...
case 'laporan': //KIRIGIRI
       if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted:fdoc})
       balas('*HALO  KACK ADA MASALAH APA DENGAN BOT NYA KAK SI LAHKAN CHAT OWNER/DEVELOPER BOT*')
...
break
			...
			case 'paper':
			       if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted:fdoc})
			if (args.length < 1) return balas(`Contoh: ${prefix}paper ${namaowner}`)
			papg = body.slice(6)
			glass = await getBuffer(`https://videfikri.com/api/textmaker/${command}/?text=${papg}`)
			ronove.sendMessage(from, glass, image, {quoted:fexec, caption: 'INI'})
			await limitAdd(sender)
break
    case 'tinyurl':
           if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted:fdoc})
...
break 
					...
					case 'rainbowshine':
					       if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted:fdoc})
                    sticWait(from)
                    if (args.length == 0) return balas(`Example: ${prefix + command} ${namabot}`)
                    teki = args.join(" ")
                    repur = await getBuffer(`https://api.xteam.xyz/photooxy/${command}?text=${teki}&APIKEY=${xteamkey}`)
                    ronove.sendMessage(from, repur, image, { quoted:fexec})
                    await limitAdd(sender)
break
        					case 'pornhub':
               if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted:fdoc})
        sticWait(from)
					 if (args.length == 0) return balas(`Example: ${prefix + command} ${namabot}|baik`)
                    query = args.join(" ")
					porn = `${body.slice(8)}`
					hub1 = porn.split("|")[0];
					hub2 = porn.split("|")[1];
					porn = await getBuffer(`https://api.xteam.xyz/textpro/ph?text=${hub1}&text2=${hub2}&APIKEY=${xteamkey}`)
					ronove.sendMessage(from, porn, image, {caption: 'PORNHUB', quoted:fexec})
					await limitAdd(sender)
break 
        case 'joox':
               if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted:fdoc})
...
break
		...
		if (!isRegistered) return balas(ind.noregis())
                ...
                case 'nekopoisearch':
                    ...
                    // Information //
                case 'heroml':
                    if (args.length == 0) return balas(`Example: ${prefix + command} Fanny`)
                    hero = args[0]
                ...
                case 'wikipedia':
		if (!isRegistered) return balas(ind.noregis())
                    if (args.length == 0) return balas(`Example: ${prefix + command} Tahu`)
                    query = args.join(" ")
...
if (budy.startsWith("$ ")){
if (!mek.key.fromMe && isOwner) return balas(`Mau eval? awokawokawok khusus owner gaada yg bisa ambil case atau apapun`)
