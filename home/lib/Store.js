...
function bind(vynnx) {
    ...
    vynnx.ev.on('connection.update', update => {
        Object.assign(state, update)
    })

    vynnx.ev.on('chats.set', ({ chats: newChats, isLatest }) => {
        /**if (isLatest) {
            chats.clear()
        }**/
    ...
    vynnx.ev.on('contacts.set', ({ contacts: newContacts }) => {
    ...
    vynnx.ev.on('messages.set', ({ messages: newMessages, isLatest }) => {
        if (isLatest) {
   ...
   vynnx.ev.on('contacts.update', updates => {
        for (const update of updates) {
            Object.assign(contacts[update.id], update)
        }
    })

    vynnx.ev.on('chats.upsert', newChats => {
        chats.upsert(...newChats)
    })

    vynnx.ev.on('messages.delete', item => {
        if ('all' in item) {
    ...
    vynnx.ev.on('chats.delete', deletions => {
        for (const item of deletions) {
            chats.deleteById(item)
        }
    })

    vynnx.ev.on('chats.update', updates => {
    ...
    vynnx.ev.on('presence.update', ({ id, presences: update }) => {
        presences[id] = presences[id] || {}
        Object.assign(presences[id], update)
    })

    vynnx.ev.on('messages.upsert', ({ messages: newMessages, type }) => {
        switch (type) {
            case 'append':
            case 'notify':
                for (const msg of newMessages) {
    ...
    /**vynnx.ev.on('messages.update', updates => {
    ...
    vynnx.ev.on('groups.update', updates => {
        for (const update of updates) {
    ...
    vynnx.ev.on('group-participants.update', ({ id, participants, action }) => {
    ...
    vynnx.ev.on('message-receipt.update', updates => {
    ...
    return {
        ...
        loadMessages: async (jid, count, cursor) => {
            ...
            const retrieve = async (count, cursor) => {
                const result = await (vynnx === null || vynnx === void 0 ? void 0 : vynnx.fetchMessagesFromWA(jid, count, cursor))
                return result || []
            }
        ...
        loadMessage: async (jid, id) => {
            ...
            if (!message) {
                message = await (vynnx === null || vynnx === void 0 ? void 0 : vynnx.loadMessageFromWA(jid, id))
            }
            return message
        },
        mostRecentMessage: async (jid) => {
            ...
            if (!message) {
                const [result] = await (vynnx === null || vynnx === void 0 ? void 0 : vynnx.fetchMessagesFromWA(jid, 1, undefined))
                message = result
            }
            return message
        },
        fetchImageUrl: async (jid) => {
            ...
            if (!contact) {
                return vynnx === null || vynnx === void 0 ? void 0 : vynnx.profilePictureUrl(jid)
            }
            if (typeof contact.imgUrl === 'undefined') {
                contact.imgUrl = await (vynnx === null || vynnx === void 0 ? void 0 : vynnx.profilePictureUrl(jid))
            }
            return contact.imgUrl
        },
        fetchGroupMetadata: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (vynnx === null || vynnx === void 0 ? void 0 : vynnx.groupMetadata(jid))
            }
            return groupMetadata[jid]
        },
        fetchBroadcastListInfo: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (vynnx === null || vynnx === void 0 ? void 0 : vynnx.getBroadcastListInfo(jid))
            }
            return groupMetadata[jid]
        },
        fetchMessageReceipts: async ({ remoteJid, id }) => {
            ...
            if (!receipts) {
                receipts = await (vynnx === null || vynnx === void 0 ? void 0 : vynnx.messageInfo(remoteJid, id))
                if (msg) {
